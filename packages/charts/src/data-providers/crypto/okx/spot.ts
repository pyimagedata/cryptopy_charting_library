 ...
 * OKX Spot Provider
 * 
 * Combined Orderbook + Candle provider for OKX Spot market.
 * Uses okx.com API endpoints.
 */
...
export interface OkxSpotConfig {
    maxOrderbookLevels?: number;
}

const DEFAULT_CONFIG: Required<OkxSpotConfig> = {
 ...
 */
export class OkxSpotProvider extends BaseDataProvider implements IOrderbookProvider, ICandleProvider {
    readonly name = 'OKX Spot';
    readonly marketType = 'spot' as const;

    private _config: Required<OkxSpotConfig>;
    ...
    constructor(config: OkxSpotConfig = {}) {
        super();
    ...
    private _processOrderbookUpdate(symbol: string, data: any): void {
        ...
        // OKX sends full updates with books channel
    ...
    async getCandles(symbol: string, interval: CandleInterval, limit: number = 300): Promise<Candle[]> {
        ...
        const response = await fetch(url);
        if (!response.ok) throw new Error(`OKX candle API error: ${response.status}`);

        const data = await response.json();
        if (data.code !== '0' || !data.data) {
            throw new Error(`OKX candle API error: ${data.msg}`);
        }
    ...
    private _toInstId(symbol: string): string {
