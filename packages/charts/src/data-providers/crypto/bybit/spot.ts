 ...
 * Bybit Spot Provider
 * 
 * Combined Orderbook + Candle provider for Bybit Spot market.
 * Uses api.bybit.com V5 API endpoints.
 */
...
const DEFAULT_CONFIG: Required<BybitSpotConfig> = {
...
/**
 * Bybit Spot Market Data Provider
 */
export class BybitSpotProvider extends BaseDataProvider implements IOrderbookProvider, ICandleProvider {
    readonly name = 'Bybit Spot';
    readonly marketType = 'spot' as const;

    private _config: Required<BybitSpotConfig>;
    ...
    constructor(config: BybitSpotConfig = {}) {
        super();
    ...
    async getOrderbook(symbol: string, limit: number = 500): Promise<Orderbook> {
        const url = `${BYBIT_REST_URL}/v5/market/orderbook?category=spot&symbol=${symbol}&limit=${Math.min(limit, this._config.maxOrderbookLevels)}`;
    ...
    async getCandles(symbol: string, interval: CandleInterval, limit: number = 200): Promise<Candle[]> {
        ...
        const url = `${BYBIT_REST_URL}/v5/market/kline?category=spot&symbol=${symbol}&interval=${intervalStr}&limit=${Math.min(limit, 200)}`;

        const response = await fetch(url);
        if (!response.ok) throw new Error(`Bybit candle API error: ${response.status}`);

        const data = await response.json();
        if (data.retCode !== 0 || !data.result?.list) {
            throw new Error(`Bybit candle API error: ${data.retMsg}`);
        }
        ...
        if (!this._subscriptions.has(`candle:${key}`)) {
            ...
            this._subscribeWs(`kline.${intervalStr}.${symbol}`, (data) => {
                if (data.data?.[0]) {
                    const c = data.data[0];
                    const candle: Candle = {
                        time: Math.floor(parseInt(c.start) / 1000),
