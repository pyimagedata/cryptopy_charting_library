/**
 * XABCD Pattern Drawing Implementation (Harmonic Pattern)
 * 
 * A 5-point harmonic pattern where:
 * - Points are labeled X, A, B, C, D
 * - Lines connect X-A, A-B, B-C, C-D
 * - Used for Gartley, Butterfly, Bat, Crab patterns
 */
...
export interface XABCDPatternOptions {
    color?: string;
    lineWidth?: number;
    lineDash?: number[];
    fillColor?: string;
...
// Point labels for XABCD pattern
export const XABCD_LABELS = ['X', 'A', 'B', 'C', 'D'];

/**
 * XABCD Pattern - 5-point harmonic pattern
 */
export class XABCDPatternDrawing implements Drawing, DrawingSettingsProvider {
    readonly id: string;
    readonly type: DrawingType = 'xabcdPattern';
    ...
    constructor(options: XABCDPatternOptions = {}) {
    ...
    isComplete(): boolean {
        return this.points.length >= 5 && this.state === 'complete';
    }
    ...
    hitTest(x: number, y: number, threshold: number = 5): boolean {
        ...
        // Check each line segment: X-A, A-B, B-C, C-D
    ...
    static fromJSON(data: SerializedDrawing): XABCDPatternDrawing {
        const drawing = new XABCDPatternDrawing({
