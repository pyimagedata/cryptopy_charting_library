/**
 * Date Range Drawing Implementation
 * 
 * A 2-point drawing tool for measuring the horizontal distance (time) between two points.
 * Shows the number of bars and the duration.
 * 
 * Points:
...
export class DateRangeDrawing implements Drawing, DrawingSettingsProvider {
    readonly id: string;
    readonly type: DrawingType = 'dateRange';
    ...
    set barCount(value: number) { this._barCount = value; }
    ...
    hitTest(x: number, y: number, threshold: number = 8): boolean {
        ...
        // For date range, the height can be arbitrary during creation or based on points
        // In most charts, it spans a good portion of the vertical space or follows the drag
    ...
    static fromJSON(data: SerializedDrawing): DateRangeDrawing {
        const drawing = new DateRangeDrawing({
