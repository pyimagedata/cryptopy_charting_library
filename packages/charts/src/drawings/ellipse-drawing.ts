...
/**
 * Ellipse - A 2-point shape defined by bounding box corners
 */
export class EllipseDrawing implements Drawing, DrawingSettingsProvider {
    ...
    /** Hit test on the ellipse border or body */
    hitTest(x: number, y: number, threshold: number = 5): boolean {
        ...
        // Inside ellipse (for fill hit)
        if (distance <= 1) return true;

        // On border (with threshold)
        const thresholdNormalized = threshold / Math.min(radiusX, radiusY);
        if (distance <= (1 + thresholdNormalized) * (1 + thresholdNormalized) &&
            distance >= (1 - thresholdNormalized) * (1 - thresholdNormalized)) {
            return true;
        }
